...
public class WebSocketChatt { // 클라이언트가 접속할 떔나다 생성되어 클라이언트와 직접 통신하는 클라스 => 1/N의 통신이 가능하도록 만들어야함
	...
	@OnOpen // 클라이언트가 브라우저를 끄거나 다른 경로로 이동할 때
	public void onOpen(Session s) { //클라이언트로부터 메시지가 전달되면 WebSocketChatt 클래스의 onMessage메서드에 의해 client에 있느 모든 session 메세지 전달받음
	...
	@OnMessage//클라이언트가 접속할 떄 발생하는 이벤트
